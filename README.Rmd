...
We find that the locations in our dataset have seen a progressive increase in average temperatures over the time since the beginning of the time period in 1962.
...
When charted by region, we can see that this increase in average temperatures is felt across all climate regions, but is most obvious in region 6, the coolest region.
...
```{r}
allData %>% filter(!is.na(`TAVG`)) %>% group_by(region) %>% ggplot(aes(x = TAVG, )) + geom_histogram(binwidth = 5, fill = 'darkred') + ggtitle('Average Temperature Histogram') + ylab("Count") + xlab("Average Temperature (F)") + theme(legend.position = "none") 
```
...
After normalizing death counts across our regions we find that death rates accross our sample set are significantly higher on avg during the winter season and lower in summer months.
